...
public class SlicedNumberTest
{
    ...
    public void SlicedNumberConstructorTest()
    {
        ...
        Assert.AreEqual(1, target.nslices);
        Assert.AreEqual(1, target.slices[0]);
        Assert.AreEqual(1, target.sign);

        target = new SlicedNumber("123456789");
        Assert.AreEqual(1, target.nslices);
        Assert.AreEqual(123456789, target.slices[0]);
        Assert.AreEqual(1, target.sign);
        ...
        Assert.AreEqual(3, target.nslices);
        Assert.AreEqual(123456789, target.slices[0]);
        Assert.AreEqual(987654321, target.slices[1]);
        Assert.AreEqual(12345, target.slices[2]);
        Assert.AreEqual(1, target.sign);
    ...
    public void NormalizeSlicesTest()
    {
        ...
        Assert.AreEqual(1, target.nslices);
        Assert.AreEqual(123, target.slices[0]);

        target.nslices = 2;
        target.slices = [123456789012, 1000];
        target.NormalizeSlices();
        Assert.AreEqual(2, target.nslices);
        Assert.AreEqual(456789012, target.slices[0]);
        Assert.AreEqual(1123, target.slices[1]);

        target.nslices = 1;
        target.slices = [123456789012];
        target.NormalizeSlices();
        Assert.AreEqual(2, target.nslices);
        Assert.AreEqual(456789012, target.slices[0]);
        Assert.AreEqual(123, target.slices[1]);

        target.nslices = 3;
        target.slices = [-1000, 1000, 1];
        target.NormalizeSlices();
        Assert.AreEqual(3, target.nslices);
        Assert.AreEqual(999999000, target.slices[0]);
        Assert.AreEqual(999, target.slices[1]);
        Assert.AreEqual(1, target.slices[2]);

        target.nslices = 3;
        target.slices = [1000, -1000, 1];
        target.NormalizeSlices();
        Assert.AreEqual(3, target.nslices);
        Assert.AreEqual(1000, target.slices[0]);
        Assert.AreEqual(999999000, target.slices[1]);
        Assert.AreEqual(0, target.slices[2]);               // Non-significant 0, normal, NormalizeSlices doesn't trim
    }
    ...
    public void TrimNonSignificantZeroesTest()
    {
        ...
        target.TrimNonSignificantZeroes();
        Assert.AreEqual(1, target.nslices);
        Assert.AreEqual(0, target.slices[0]);

        target.nslices = 2;
        target.slices = [123456, 0];
        target.TrimNonSignificantZeroes();
        Assert.AreEqual(1, target.nslices);
        Assert.AreEqual(123456, target.slices[0]);
    }
    ...
    public void AbsCompareTest()
    {
        SlicedNumber n1 = new("8");
        SlicedNumber n2 = new("7");
        Assert.AreEqual(1, SlicedNumber.AbsCompare(n1, n2));
        ...
        Assert.AreEqual(0, SlicedNumber.AbsCompare(n1, n2));
    }
