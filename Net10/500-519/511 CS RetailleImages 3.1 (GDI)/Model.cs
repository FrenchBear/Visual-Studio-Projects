...
#pragma warning restore IDE0052 // Remove unread private members
        ...
        _ = Task.Run(/* async */ () =>
          {
              ...
              foreach (var file in processedFilesList)
              {
                  ...
                  var s = file[(SourceFolder.Length + (SourceFolder.EndsWith('\\') ? 0 : 1))..];    // Avoid problems with loop variables
    ...
    public string ConvertImage(string fileName)
    {
        ...
        if (Path.GetDirectoryName(fileName).EndsWith(" HR", StringComparison.InvariantCultureIgnoreCase))
        {
            ...
            fileNameHRStripped = Path.Combine(d[..^3], Path.GetFileName(fileName));
        }
        else
