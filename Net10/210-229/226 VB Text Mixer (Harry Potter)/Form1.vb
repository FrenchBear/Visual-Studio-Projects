...
Public Class Form1
    ...
    Friend Declare Sub QueryPerformanceFrequency Lib "Kernel32.dll" (ByRef freq As Long)
    ...
    Private ReadOnly tdSingleProb(126 - 32) As Double     ' Cumuled probability
    Private ReadOnly tiSingleChar(126 - 32) As Integer    ' Character code
    Private ReadOnly tdDoubleProb(126 - 32)() As Double     ' Cumuled probability
    Private ReadOnly tiDoubleChar(126 - 32)() As Integer    ' Character code



    Public Shared Function IsFiller(i As Integer) As Boolean
        Static aldRand As New ArrayList
        If i >= aldRand.Count Then aldRand.Add(Rnd)
        IsFiller = aldRand.Item(i) < 0
    End Function

    Public Sub Encrypt(s As String)
        Dim i, j, i0 As Integer, c As Integer, bFiller As Boolean
        Do
            If IsFiller(i0) Then
                c = If(i = 0, GetRandomChar(), If(IsFiller(i0 + 1) Or i >= s.Length, GetRandomChar(c), GetRandomChar(c, Asc(s.Chars(i)))))
                bFiller = True
            Else
    ...
    Public Sub Analyze()
        ...
        For i = 32 To 126
            tiSingleChar(i - 32) = i
            tdSingleProb(i - 32) = If(tlSingle(i - 32) = 0, -1.0, tlSingle(i - 32) / lTot)
        ...
        For i = 32 To 126
            ...
            ti = Array.Empty(Of Integer)()
            ReDim ti(126 - 32)
            td = Array.Empty(Of Double)()
            ReDim td(126 - 32)

            For j = 32 To 126
                ti(j - 32) = j
                td(j - 32) = If(tlPairs(i - 32, j - 32) = 0, -1.0, tlPairs(i - 32, j - 32) / lTot)
            Next
    ...
    ' Array.Sort: 100ms
    ' ArraySort2: 4ms
    Public Shared Sub ArraySort2(ByRef td() As Double, ByRef ti() As Integer)
    ...
    Public Function GetRandomChar() As Integer
    ...
    Public Function GetRandomChar(FirstChar As Integer) As Integer
        Static myDoubleAscComparer As New DoubleAscComparer
        Dim j As Integer
    ...
    Public Function GetRandomChar(FirstChar As Integer, NextChar As Integer) As Integer
        Static myDoubleAscComparer As New DoubleAscComparer
        Dim j, k As Integer
        Do
            j = Array.BinarySearch(tdDoubleProb(FirstChar - 32), Rnd(), myDoubleAscComparer)
    ...
    Private Shared iCall As Integer
    ...
    Public Class TempClassDescComparer
...
Friend Class DoubleDescComparer
    Implements IComparer
...
Friend Class DoubleDescComparer2
    Implements IComparer
...
Friend Class DoubleAscComparer
    Implements IComparer
