...
Module modRenduGraphique
    ...
    Function RGTextOut(sFontName As String, fFontSize As Single, bFontBold As Boolean, bFontItalic As Boolean, bFontUnderline As Boolean, bFontNarrow As Boolean, x As Integer, y As Integer, sText As String) As Integer
        ...
        ' Ancien calcul de la taille, mais MeasureString d�borde...
    ...
    Sub RGPaintBitmap(x As Integer, y As Integer, sNomfic As String)
        ...
        Catch ex As Exception
            Console.WriteLine("�chec � la lecture de {0}: {1}", sNomfic, ex.ToString)
            Exit Sub
        End Try
...
Module modRenduPCL
    ...
    Class PCLState
        Public posX, posY As Integer              ' Coordonn�es en decipoints
        Public VMI As Integer                     ' Hauteur d'une ligne en decipoints
        Public HMI As Integer                     ' Largeur d'un caract�re en d�cipoints
        Public iLeftOffset, iTopOffset As Integer ' D�calage de l'origine en d�cipoints
    ...
    Sub PCLInitJob()
        ...
        With staState
            ...
            .VMI = 720 / 6  ' 6LPI par d�faut
            .HMI = 720 / 10 ' 10 CPI par d�faut
            .sFont = "Courier New"
            .fFontSize = 10
            .bFontBold = False
            .bFontItalic = False
    ...
    ' Imprime un caract�re imprimable
    ' En pratique, buff�rise le caract�re
    Sub PCLPrint(b As Byte)
        Select Case b
            ...
            Case 9
                ' Tabs � �tendre - pour l'instant, on simplifie
                PCLFlushText()
            ...
            Case 13
                ' CR
                PCLFlushText()
                ' Retour du curseur � la marge texte de gauche
                staState.posX = staState.iLeftMargin
    ...
    ' "Imprime" le texte buff�ris� par PCLPrint
    Sub PCLFlushText()
    ...
    ' "Ejecte" la page PCL = ferme la page du rendu graphique, et met � jour l'�tat PCL
    Sub PCLFlushPage()
    ...
    Sub PCL38aL(iMarge As Integer)
        TraceWrite("[Marge gauche texte = {0} caract�res]", iMarge)
    End Sub
    ...
    Sub PCL38dD(iTypeSoul As Integer)
        Select Case iTypeSoul
            ...
            Case Else
                PCLError("S�quence <Esc>&d" & iTypeSoul & "D inconnue")
        End Select
    End Sub
    ...
    Sub PCL40U(iJeuCar As Integer)
        TraceWrite("[Jeu de caract�res {0}]", iJeuCar)
        Select Case iJeuCar
            Case 10
                ' PC-8
            Case Else
                PCLError("Jeu de caract�res <Esc>(" & iJeuCar & "U non pris en charge")
        End Select
    End Sub
    ...
    Sub PCL40sP(iType As Integer)
        Select Case iType
            Case 0
                TraceWrite("[S�lection police fixe]")
                staState.sFont = "Courier New"
            Case 1
                TraceWrite("[S�lection police proportionnelle]")
                staState.sFont = "Arial"
            Case Else
                PCLError("S�quence <Esc>(s" & iType & "P inconnue")
        End Select
    End Sub

    ' <Esc>(s#H
    Sub PCL40sH(fPitch As Single)
        TraceWrite("[S�lection pas horizontal police {0} points/pouce]", fPitch)
        staState.fFontSize = 120 / fPitch
    End Sub

    ' <Esc>(s#V
    Sub PCL40sV(fHauteur As Single)
        TraceWrite("[S�lection hauteur police {0} points]", fHauteur)
    ...
    Sub PCL40sS(iItalique As Single)
        Select Case iItalique
            Case 0
                TraceWrite("[S�lection police normale (pas italique)]")
                staState.bFontItalic = False
                staState.bFontNarrow = False
            Case 1
                TraceWrite("[S�lection police italique]")
                staState.bFontItalic = True
            Case 4
                TraceWrite("[S�lection police narrow]")
                staState.bFontNarrow = True
            Case Else
                PCLError("S�quence <Esc>(s" & iItalique & "S inconnue")
        End Select
    End Sub
    ...
    Sub PCL40sB(iGraise As Integer)
        Select Case iGraise
            Case 0
                TraceWrite("[S�lection police normale (non grasse)]")
                staState.bFontBold = False

            Case 3
                TraceWrite("[S�lection police grasse")
                staState.bFontBold = True

            Case Else
                TraceWrite("[Selection police graisse {0}]", iGraise)
                PCLError("S�quence <Esc>(s" & iGraise & "B non prise en charge")
        End Select
    End Sub

    ' <Esc>(s#T
    Sub PCL40sT(iFont As Integer)
        TraceWrite("[S�lection police n� {0}]", iFont)
    ...
    Sub PCL42rA(iLeftPos As Integer)
        Select Case iLeftPos
            ...
            Case Else
                PCLError("S�quence <Esc>*r" & iLeftPos & "A inconnue")
                Exit Sub
        End Select
    ...
    Sub PCL42bW(iBytes As Integer)
        TraceWrite("[S�quence graphique {0} octets]", iBytes)
    End Sub
...
' On travaille sur les s�quences d'octets, c'est ce qu'il y a de plus rapide
...
Module modConversionImage1Bit
    ...
    Public Function ImgTo1Bit(ByRef imgSource As Bitmap) As Bitmap
        ...
        For y As Integer = 0 To b1.Height - 1
            ...
            ' On Récup�re une ligne de l'image couleur
