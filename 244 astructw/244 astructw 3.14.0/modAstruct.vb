...
Label1:
    ...
    Private Sub Enumerate(ByVal sPath As String, ByVal colFoldersSource As Kollection(Of String), ByVal colFilesSource As Kollection(Of MyFileInfo))
        ...
        'If sPath.IndexOf("servicing", StringComparison.InvariantCultureIgnoreCase) >= 0 Then Stop
        For Each sExcl As String In colExclusionsPaths
            If sPath.IndexOf(sExcl, StringComparison.InvariantCultureIgnoreCase) >= 0 Then
                If bVerbose Then Trace("Skip enumaration of folder " & Quote(sPath) & " because of path exclusion match")
                Exit Sub
            End If
        Next
                ...
                If findinfo.dwFileAttributes And FileAttributes.Directory Then
                    ...
                    For Each sExcl As String In colExclusionsPaths
                        If (sPath & findinfo.cFileName & "\").IndexOf(sExcl, StringComparison.InvariantCultureIgnoreCase) >= 0 Then
                            If bVerbose Then Trace("Ignored folder " & Quote(sPath & findinfo.cFileName) & " because of path exclusion match")
                            GoTo NextFile
                        End If
                    Next
