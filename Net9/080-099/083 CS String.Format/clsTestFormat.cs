...
internal readonly struct Complex(double r, double i): IFormattable
{
    private readonly double x = r, y = i;
...
#pragma warning disable JSON002 // Probable JSON string detected
        ...
        DoOutput("Formats de dates", DateTime.Now, ["{0:dddd dd MMMM yyyy}", "{0:dd/MM/yy}", "{0:hh:mm:ss}"]);
        DoOutput("Formats de types spécifiques", new Complex(1, 2), ["{0}", "{0:P}", "{0:R}"]);
        ...
        DoOutput("Format spécifiques d'entiers", 1234567, ["{0:#,##0}", "{0:0,}", "{0:#,##0,}", "{0:0.00%}"]);

        WriteLine("Passage en culture US");
        Thread.CurrentThread.CurrentCulture = new CultureInfo("en-us");
        DoOutput("Formats de décimaux", 3141.5926, ["{0:G}", "{0:C}"]);
        DoOutput("Formats de dates", DateTime.Now, ["{0:dddd dd MMMM yyyy}"]);
