...
Friend Module modPCL
    ...
    Private epclEtat As EtatPCL                 ' Etat courant de l'automate

    Private epclNextEtat As EtatPCL
    Private iDataCount As Integer               ' Nb d'octets de séquences de données
    Private sFactor As String                   ' Facteur de répétition des commandes PCL
    Private bMainFunction As Byte               ' Fonction qui suit immédiatement Esc */&/(
    Private tbData() As Byte                ' Flot de données transparentes


    Public Sub InitEtatPCL()
        epclEtat = EtatPCL.epclPrint
        iDataCount = 0
    End Sub

    Public Sub AnaPCL(b As Byte)
    ...
    Public Sub AnaPCLEscape(b As Byte)
        ...
        epclNextEtat = If(b >= Asc("@") And b <= Asc("Z"), EtatPCL.epclPrint, epclEtat)
    ...
    Public Sub PCLFunction(epclEtat As EtatPCL, cMainFunction As Char, cFunction As Char, sFactor As String)
        TraceWrite("<Esc>")
        Select Case epclEtat
    ...
    ' Fonctions Esc &
    Public Sub PCL38(cMainFunction As Char, cFunction As Char, sFactor As String)
        Select Case cMainFunction & cFunction
            Case "?F" : PCL38F()
    ...
    ' Fonctions Esc (
    Public Sub PCL40(cMainFunction As Char, cFunction As Char, sFactor As String)
        Select Case cMainFunction & cFunction
    ...
    ' Fonctions Esc *
    Public Sub PCL42(cMainFunction As Char, cFunction As Char, sFactor As String)
        Select Case cMainFunction & cFunction
            Case "?C" : PCL42C()
...
Friend Module modMacros
    ...
    Public Sub PCLLearn(b As Byte)
        Debug.Assert(bLearningMacro)
        macCurrentMacro.sMacroText.Append(Chr(b))
    End Sub

    Public Sub PCLStopLearning()
    ...
    ' <Esc>&f#Y
    Public Sub PCL38fY(iId As Integer)
        TraceWrite("[Macros ID #{0}]", iId)
        iMacroID = iId
    End Sub

    ' <Esc>&f#X
    Public Sub PCL38fX(iFonction As Integer)
...
Friend Module modPCLError
    Public Sub PCLError(s As String)
        WriteLine("Erreur PCL pos {0}: {1}", GetPosition, s)
    End Sub

    Public Sub PCLError(format As String, arg0 As Object)
...
Friend Module modTilda
    Private sTildaText As StringBuilder

    Public Sub TildaClearBuffer()
        sTildaText = New StringBuilder
    End Sub

    Public Sub TildaLearn(b As Byte)
        sTildaText.Append(Chr(b))
    End Sub

    Public Sub TildaExecute()
