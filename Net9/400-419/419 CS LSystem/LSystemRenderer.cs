...
public abstract class LSystemRenderer(IEnumerable<char> s, int angle)
{
    ...
    public Rect Rend(double rendingWidth, double rendingHeight)
    {
        ...
        for (; ; pass++)
        {
                ...
                switch (c)
                {
                    ...
                    case '|':
                        if (_angle % 2 == 0)
                            ...
                            ap.Angle += generalOrientation * (_angle / 2) * angleIncrement;
                        break;
            ...
            rx = Math.Abs(xmax - xmin) < 1e-8 ? 100000 : rendingWidth / (1.1 * (xmax - xmin));
            ry = Math.Abs(ymax - ymin) < 1e-8 ? 100000 : rendingHeight / (1.1 * (ymax - ymin));
            r = Math.Min(rx, ry);
